...
"""
    ...
    def test_la_gomera_instancia_general_profile_binds_exact_public_launch_without_sensitive_mechanisms(self) -> None:
        catalog = GENERATOR.generate(SOURCE, SITE_PROFILES)
        target = next(
            entry for entry in catalog["entries"]
            if entry["inventoryId"] == "ES-PUB-0131"
        )
        self.assertEqual("la-gomera-instancia-general", target["profileId"])
        self.assertEqual(
        ...
        self.assertNotIn("launchUrl", target)
        self.assertEqual(
            "LA_GOMERA_SEDE_INSTANCIA_GENERAL_PUBLIC_LAUNCH",
    ...
    def test_lanzarote_instancia_general_profile_binds_exact_public_launch_without_sensitive_mechanisms(self) -> None:
        catalog = GENERATOR.generate(SOURCE, SITE_PROFILES)
        target = next(
            entry for entry in catalog["entries"]
            if entry["inventoryId"] == "ES-PUB-0135"
        )
        self.assertEqual("lanzarote-instancia-general", target["profileId"])
        self.assertEqual(
            "https://lanzaroteylagraciosa.sedelectronica.es/catalog/tw/5161fa8d-970e-4b48-a506-b2ac34ceafe5",
            target["entryUrl"],
        )
        self.assertNotIn("launchUrl", target)
        self.assertEqual(
            "LANZAROTE_INSTANCIA_GENERAL_PUBLIC_LAUNCH",
            target["protocolFamily"],
        )
        self.assertEqual("E2E_PENDING", target["catalogStatus"])
        self.assertEqual("IMPLEMENTED_NOT_E2E", target["inventoryStatus"])
    ...
    def test_fega_solicitud_general_binds_exact_qa_launch_without_signing_capability(self) -> None:
        ...
        self.assertIn("e2e", target["limitations"].lower())
    def test_almeria_solicitud_general_profile_binds_exact_qa_launch_without_sensitive_mechanisms(self) -> None:
        catalog = GENERATOR.generate(SOURCE, SITE_PROFILES)
        target = next(entry for entry in catalog["entries"] if entry["inventoryId"] == "ES-PUB-0142")

        self.assertEqual("diputacion-almeria-portal", target["portalId"])
        self.assertEqual("diputacion-almeria-solicitud-general", target["profileId"])
        self.assertEqual(
        ...
        self.assertNotIn("launchUrl", target)
        self.assertEqual(
            "DIPALME_OFICINA_VIRTUAL_SOLICITUD_GENERAL_QA_LAUNCH",
            target["protocolFamily"],
        )
        self.assertEqual("E2E_PENDING", target["catalogStatus"])
        self.assertEqual("IMPLEMENTED_NOT_E2E", target["inventoryStatus"])
    ...
    def test_caceres_instancia_general_binds_exact_qa_launch_without_signing_capability(self) -> None:
        catalog = GENERATOR.generate(SOURCE, SITE_PROFILES)
        target = next(entry for entry in catalog["entries"] if entry["inventoryId"] == "ES-PUB-0147")
        self.assertEqual("diputacion-caceres-portal", target["portalId"])
        self.assertEqual("diputacion-caceres-instancia-general", target["profileId"])
        self.assertEqual(
            "https://sede.dip-caceres.es/carpetaCiudadano/fichaprocedimiento.do?idproc=341",
            target["entryUrl"],
        )
        self.assertNotIn("launchUrl", target)
        self.assertEqual("DIP_CACERES_CLAVE_PUBLIC_LAUNCH", target["protocolFamily"])
        self.assertEqual("E2E_PENDING", target["catalogStatus"])
        self.assertEqual("IMPLEMENTED_NOT_E2E", target["inventoryStatus"])
        self.assertEqual("REVIEWED", target["discoveryState"])
        self.assertEqual("2026-08-21", target["reviewedOn"])
    ...
    def test_diputacion_sevilla_clave_client_tls_profile_binds_exact_catalog_contract(self) -> None:
        catalog = GENERATOR.generate(SOURCE, SITE_PROFILES)
        target = next(entry for entry in catalog["entries"] if entry["inventoryId"] == "ES-PUB-0170")

        self.assertEqual("diputacion-sevilla-sede", target["portalId"])
        self.assertEqual("diputacion-sevilla-sede", target["profileId"])
        self.assertEqual("https://sedeelectronicadipusevilla.es", target["entryUrl"])
        self.assertNotIn("launchUrl", target)
        self.assertEqual("CLIENT_TLS_AUTH_CLAVE", target["protocolFamily"])
        self.assertEqual("E2E_PENDING", target["catalogStatus"])
        self.assertEqual("IMPLEMENTED_NOT_E2E", target["inventoryStatus"])
        ...
        self.assertEqual([], target["observedSignatureFormats"])
        self.assertIn("qa-only", target["limitations"].lower())
        self.assertIn("autofirma", target["limitations"].lower())
    ...
    def test_enaire_public_sede_binds_exact_qa_boundary_without_sensitive_capability(self) -> None:
        catalog = GENERATOR.generate(SOURCE, SITE_PROFILES)
        target = next(entry for entry in catalog["entries"] if entry["inventoryId"] == "ES-PUB-0044")

        self.assertEqual("enaire-sede-public", target["profileId"])
        self.assertEqual("https://enaire.sede.gob.es/", target["entryUrl"])
        self.assertNotIn("launchUrl", target)
        self.assertEqual("ENAIRE_CLAVE_AUTOFIRMA_PUBLIC_BOUNDARY", target["protocolFamily"])
        self.assertEqual("E2E_PENDING", target["catalogStatus"])
        self.assertEqual("IMPLEMENTED_NOT_E2E", target["inventoryStatus"])
        self.assertEqual("REVIEWED", target["discoveryState"])
        self.assertEqual("2026-08-24", target["reviewedOn"])
    ...
    def test_fnmt_general_form_reg_age_alias_binds_exact_qa_launch(self) -> None:
        catalog = GENERATOR.generate(SOURCE, SITE_PROFILES)
    ...
    def test_csd_public_sede_binds_exact_qa_boundary_without_sensitive_capability(self) -> None:
        catalog = GENERATOR.generate(SOURCE, SITE_PROFILES)
        target = next(entry for entry in catalog["entries"] if entry["inventoryId"] == "ES-PUB-0036")

        self.assertEqual("csd-sede-public", target["profileId"])
        self.assertEqual("https://sede.csd.gob.es/", target["entryUrl"])
        self.assertNotIn("launchUrl", target)
        self.assertEqual("CSD_AUTOFIRMA_PUBLIC_BOUNDARY", target["protocolFamily"])
        self.assertEqual("E2E_PENDING", target["catalogStatus"])
        self.assertEqual("IMPLEMENTED_NOT_E2E", target["inventoryStatus"])
        self.assertEqual("REVIEWED", target["discoveryState"])
        self.assertEqual("2026-08-23", target["reviewedOn"])
