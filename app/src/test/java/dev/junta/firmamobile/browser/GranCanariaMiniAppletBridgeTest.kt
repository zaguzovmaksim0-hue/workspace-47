...
class GranCanariaMiniAppletBridgeTest {
    @Test
    fun acceptsTheExactPublicGranCanariaPadesTuple() {
        val pdf = "%PDF-1.4\n% synthetic Gran Canaria request\n".encodeToByteArray()
    ...
    fun rejectsWrongPageAndAnyNonExactPadesTuple() {
        ...
        assertEquals(true, route(message(pdf).replace("filters=nonexpired:", "filters=expired:")) is MiniAppletBridgeRouteResult.Rejected)
        pdf.fill(0)
    }
    ...
    private companion object {
        ...
        const val PROTOCOL_ID = "gran-canaria-direct-pades-v1"
        const val ORIGIN = "https://sede.grancanaria.com"
        const val SIGNING_PAGE_URL = "https://sede.grancanaria.com/sede-privado/auth"
        const val EXTRA_PROPERTIES = "headless=true\nfilters=nonexpired:"
    }
}
