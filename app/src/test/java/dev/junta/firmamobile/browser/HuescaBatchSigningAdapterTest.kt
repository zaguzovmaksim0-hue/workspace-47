...
class HuescaBatchSigningAdapterTest {
    @Test
    fun exactHuescaBridgeOwnershipNormalizesToItsDistinctProtocolOnly() {
        val bridge = HuescaBatchBridgeAdapter(
            activeProfileId = { ProfileId(HuescaBatchBridgeAdapter.PROFILE_ID) },
        )
        val routed = bridge.route(
            rawMessage = huescaEnvelope(),
            sourceOrigin = Uri.parse(HuescaBatchBridgeAdapter.SOURCE_ORIGIN),
            isMainFrame = true,
            navigationEpoch = 9L,
        ...
        val normalized = checkNotNull(
            HuescaBatchSigningAdapter(
                registry = huescaRegistry(),
                clock = Clock.fixed(observedAt, ZoneOffset.UTC),
            ).normalize(routed.request),
        )
        assertEquals(HuescaBatchProtocolAdapter.ID, normalized.protocolId)
        assertEquals("diputacion-huesca-portal", normalized.context.profileId)
        assertEquals(1, normalized.context.profileVersion)
        assertEquals("ovc24.dphuesca.es", normalized.context.origin.host)
        assertEquals(9L, normalized.context.navigationEpoch)
        assertEquals(observedAt, normalized.context.observedAt)
        normalized.close()

        val wrongOrigin = HuescaBatchBridgeAdapter(
            activeProfileId = { ProfileId(HuescaBatchBridgeAdapter.PROFILE_ID) },
        ).route(
            rawMessage = huescaEnvelope(),
        ...
        val wrongProfile = HuescaBatchBridgeAdapter(
    ...
    private fun huescaRegistry() = BuiltInSiteProfiles.qaRegistry

    private fun huescaEnvelope(): String = JSONObject()
