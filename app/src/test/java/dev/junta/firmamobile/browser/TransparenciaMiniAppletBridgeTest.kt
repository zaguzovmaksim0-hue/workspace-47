...
class TransparenciaMiniAppletBridgeTest {
    @Test
    fun acceptsTheExactProtectedTransparenciaPadesTuple() {
        val pdf = "%PDF-1.4\n% synthetic Transparencia request\n".encodeToByteArray()
    ...
    fun rejectsWrongPageAndAnyNonExactPadesTuple() {
        ...
        assertEquals(true, route(message(pdf), "$ORIGIN/procedimiento/firma?idProc=133628&idBorr=12345&idAmb=999999") is MiniAppletBridgeRouteResult.Rejected)
        assertEquals(true, route(message(pdf), "$SIGNING_PAGE_URL&extra=1") is MiniAppletBridgeRouteResult.Rejected)
        assertEquals(true, route(message(pdf).replace("SHA512withRSA", "SHA256withRSA")) is MiniAppletBridgeRouteResult.Rejected)
        assertEquals(true, route(message(pdf).replace("PAdES", "CAdES")) is MiniAppletBridgeRouteResult.Rejected)
        assertEquals(true, route(message(pdf).replace("filters=nonexpired:true;", "filters=expired:true;")) is MiniAppletBridgeRouteResult.Rejected)
        pdf.fill(0)
    }
    ...
    private companion object {
        ...
        const val PROTOCOL_ID = "transparencia-direct-pades-v1"
        const val ORIGIN = "https://transparencia.sede.gob.es"
        const val SIGNING_PAGE_URL = "https://transparencia.sede.gob.es/procedimiento/firma?idProc=133628&idBorr=12345&idAmb=101524"
        const val EXTRA_PROPERTIES = "filters=nonexpired:true;\nheadless=true"
    }
}
