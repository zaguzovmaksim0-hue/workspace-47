...
class ItjProfileCatalogBindingTest {
    private val profileId = ProfileId("itj-sede-public-navigation")
    private val portalId = PortalId("age-instituto-para-la-transicion-justa-itj")
    private val startUrl = URI("https://sede.transicionjusta.gob.es/")
    ...
    fun profileConfigPreservesExactFailClosedNavigationContract() {
        ...
        assertEquals(
            "Sede electrónica pública del Instituto para la Transición Justa",
            profile.displayName,
        )
        ...
        assertEquals(
            setOf(ExactOrigin.parse("https://sede.transicionjusta.gob.es")),
    ...
    fun registryResolvesOnlySameOriginBrowsePathsInQa() {
        ...
        assertEquals(
            TrustMode.TRUSTED_BROWSE,
            BuiltInSiteProfiles.qaRegistry.resolve(URI("https://sede.transicionjusta.gob.es/procedimientos"))?.trustMode,
        )

        listOf(
            "http://sede.transicionjusta.gob.es/",
            "https://sede.transicionjusta.gob.es:8443/",
            "https://evil.sede.transicionjusta.gob.es/",
            "https://transicionjusta.gob.es/",
            "https://user@sede.transicionjusta.gob.es/",
        ).forEach { rejectedUrl ->
    ...
    fun securityAndProtocolIsolationEnforcesFailClosedNavigation() {
        ...
        assertEquals(
            setOf("sede.transicionjusta.gob.es"),
            JuntaOriginPolicy.browserAllowedHosts(profileId),
        )
    }

    @Test
    fun catalogBindsItjAsPendingNavigationAndResolvesLaunchOnlyForExactSeed() {
        val publicCatalog = loadBundledPublicPortalCatalog()
        val entry = publicCatalog.entries.single { it.inventoryId == "ES-PUB-0057" }

        assertEquals(portalId, entry.portalId)
        assertEquals(profileId, entry.profileId)
        assertEquals("https://sede.transicionjusta.gob.es/", entry.entryUrl.toString())
        ...
        listOf(
            ...
            "https://sede.transicionjusta.gob.es:8443/",
            "https://evil.sede.transicionjusta.gob.es/",
            "https://sede.transicionjusta.gob.es/procedimientos",
        ).forEach { rejectedUrl ->
