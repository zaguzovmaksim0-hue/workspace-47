...
class IsfasProfileCatalogBindingTest {
    private val profileId = ProfileId("isfas-sede-public-navigation")
    private val portalId = PortalId("age-instituto-social-de-las-fuerzas-armadas-isfas")
    private val startUrl = URI("https://sede.isfas.gob.es/")
    ...
    fun profileConfigPreservesExactFailClosedNavigationContract() {
        ...
        assertEquals(
            "Sede electrónica pública del Instituto Social de las Fuerzas Armadas",
            profile.displayName,
        )
        ...
        assertEquals(
            setOf(ExactOrigin.parse("https://sede.isfas.gob.es")),
    ...
    fun registryResolvesExactEntryAndSameOriginBrowsePathsInQa() {
        ...
        assertEquals(
            ...
            BuiltInSiteProfiles.qaRegistry.resolve(
                URI("https://sede.isfas.gob.es/ispre/index.html"),
            )?.trustMode,
        )

        listOf(
            "http://sede.isfas.gob.es/",
            "https://sede.isfas.gob.es:8443/",
            "https://evil.sede.isfas.gob.es/",
            "https://isfas.gob.es/",
            "https://user@sede.isfas.gob.es/",
        ).forEach { rejectedUrl ->
    ...
    fun securityAndProtocolIsolationEnforcesFailClosedNavigation() {
        ...
        assertEquals(
            setOf("sede.isfas.gob.es"),
            JuntaOriginPolicy.browserAllowedHosts(profileId),
        )
    }

    @Test
    fun catalogBindsIsfasAsPendingNavigationAndResolvesLaunchOnlyForExactEntry() {
        val publicCatalog = loadBundledPublicPortalCatalog()
        val entry = publicCatalog.entries.single { it.inventoryId == "ES-PUB-0058" }
