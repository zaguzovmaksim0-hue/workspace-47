...
class DiputacionSalamancaProfileCatalogBindingTest {
    private val profileId = ProfileId("diputacion-salamanca-sede")
    private val portalId = PortalId("diputacion-salamanca-sede")
    private val startUrl = URI("https://sede.diputaciondesalamanca.gob.es")
    ...
    fun `profile exposes only the exact reviewed browse surface`() {
        ...
        assertEquals(setOf(ExactOrigin.parse("https://sede.diputaciondesalamanca.gob.es")), profile.initiatorOrigins)
        ...
        listOf(
            "http://sede.diputaciondesalamanca.gob.es",
            "https://user@sede.diputaciondesalamanca.gob.es",
            "https://sede.diputaciondesalamanca.gob.es.evil.example",
            "https://sede.diputaciondesalamanca.gob.es:8443",
        ).forEach { rejected ->
    ...
    fun `catalog binding does not invent signature capabilities`() {
        ...
        assertEquals("ES-PUB-0168", entry.inventoryId)
