...
class DiputacionMalagaProfileCatalogBindingTest {
    private val profileId = ProfileId("diputacion-malaga-sede")
    private val portalId = PortalId("diputacion-malaga-sede")
    private val startUrl = URI("https://sede.malaga.es")
    ...
    fun `profile exposes only the exact reviewed browse surface`() {
        ...
        listOf(
            "http://sede.malaga.es",
            "https://user@sede.malaga.es",
            "https://sede.malaga.es.evil.example",
            "https://sede.malaga.es:8443",
            "https://malaga.es",
        ).forEach { rejected ->
    ...
    @Test
    fun `catalog binds Diputacion de Malaga without inventing signature capabilities`() {
        ...
        assertEquals("ES-PUB-0164", entry.inventoryId)
