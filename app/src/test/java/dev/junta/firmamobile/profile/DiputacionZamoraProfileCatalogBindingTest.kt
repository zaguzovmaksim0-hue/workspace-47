...
class DiputacionZamoraProfileCatalogBindingTest {
    private val profileId = ProfileId("diputacion-zamora-sede")
    private val portalId = PortalId("diputacion-zamora-sede")
    private val startUrl = URI("https://diputaciondezamora.sedelectronica.es")
    ...
    fun `profile exposes only the exact reviewed browse surface`() {
        ...
        assertEquals(setOf(ExactOrigin.parse("https://diputaciondezamora.sedelectronica.es")), profile.initiatorOrigins)
        ...
        listOf(
            ...
            "https://user@diputaciondezamora.sedelectronica.es",
            "https://diputaciondezamora.sedelectronica.es.evil.example",
            "https://diputaciondezamora.sedelectronica.es:8443",
        ).forEach { rejected ->
    ...
    fun `catalog binding does not invent signature capabilities`() {
        ...
        assertEquals("ES-PUB-0177", entry.inventoryId)
