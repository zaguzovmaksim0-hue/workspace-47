...
class DiputacionOurenseProfileCatalogBindingTest {
    ...
    private val startUrl = URI("https://sede.depourense.es")
    ...
    fun `profile exposes only the exact reviewed browse surface`() {
        ...
        listOf(
            "http://sede.depourense.es",
            "https://user@sede.depourense.es",
            "https://sede.depourense.es.evil.example",
            "https://sede.depourense.es:8443",
        ).forEach { rejected ->
    ...
    fun `catalog binding does not invent signature capabilities`() {
        ...
        assertEquals("ES-PUB-0165", entry.inventoryId)
