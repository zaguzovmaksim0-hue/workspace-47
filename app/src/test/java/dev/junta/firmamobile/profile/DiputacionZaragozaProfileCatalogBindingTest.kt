...
class DiputacionZaragozaProfileCatalogBindingTest {
    private val profileId = ProfileId("diputacion-zaragoza-sede")
    private val portalId = PortalId("diputacion-zaragoza-sede")
    private val startUrl = URI("https://dpz.sedelectronica.es")
    ...
    fun `profile exposes only the exact reviewed browse surface`() {
        ...
        assertEquals(setOf(ExactOrigin.parse("https://dpz.sedelectronica.es")), profile.initiatorOrigins)
        ...
        listOf(
            ...
            "https://user@dpz.sedelectronica.es",
            "https://dpz.sedelectronica.es.evil.example",
            "https://dpz.sedelectronica.es:8443",
        ).forEach { rejected ->
    ...
    fun `catalog binding does not invent signature capabilities`() {
        ...
        assertEquals("ES-PUB-0178", entry.inventoryId)
