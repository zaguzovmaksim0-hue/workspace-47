...
class DiputacionSegoviaProfileCatalogBindingTest {
    private val profileId = ProfileId("diputacion-segovia-sede")
    private val portalId = PortalId("diputacion-segovia-sede")
    private val startUrl = URI("https://sede.dipsegovia.es")
    ...
    fun `profile exposes only the exact reviewed browse surface`() {
        ...
        listOf(
            "http://sede.dipsegovia.es",
            "https://user@sede.dipsegovia.es",
            "https://sede.dipsegovia.es.evil.example",
            "https://sede.dipsegovia.es:8443",
        ).forEach { rejected ->
    ...
    fun `catalog binding does not invent signature capabilities`() {
        ...
        assertEquals("ES-PUB-0169", entry.inventoryId)
