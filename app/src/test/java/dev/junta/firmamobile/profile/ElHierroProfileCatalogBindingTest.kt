...
class ElHierroProfileCatalogBindingTest {
    private val profileId = ProfileId("el-hierro-sede-public-navigation")
    private val portalId = PortalId("el-hierro-sede-electronica")
    private val startUrl = URI("https://elhierro.sedelectronica.es/info.0")
    ...
    fun profileConfigPreservesExactFailClosedNavigationContract() {
        ...
        assertEquals(
            "Sede electrónica pública del Cabildo Insular de El Hierro",
            profile.displayName,
        )
        ...
        assertEquals(
            setOf(ExactOrigin.parse("https://elhierro.sedelectronica.es")),
    ...
    fun registryResolvesOnlyExactEntryInQaWhenOriginIsShared() {
        ...
        assertNull(BuiltInSiteProfiles.qaRegistry.resolve(URI("https://elhierro.sedelectronica.es/info")))

        listOf(
            "http://elhierro.sedelectronica.es/info.0",
            "https://elhierro.sedelectronica.es:8443/info.0",
            "https://evil.elhierro.sedelectronica.es/info.0",
            "https://sedelectronica.es/info.0",
            "https://user@elhierro.sedelectronica.es/info.0",
        ).forEach { rejectedUrl ->
    ...
    fun securityAndProtocolIsolationEnforcesFailClosedNavigation() {
        ...
        assertEquals(
            setOf("elhierro.sedelectronica.es"),
            JuntaOriginPolicy.browserAllowedHosts(profileId),
        )
    }

    @Test
    fun catalogBindsElHierroAsPendingNavigationAndResolvesLaunchOnlyForExactEntry() {
        val publicCatalog = loadBundledPublicPortalCatalog()
        val entry = publicCatalog.entries.single { it.inventoryId == "ES-PUB-0126" }
