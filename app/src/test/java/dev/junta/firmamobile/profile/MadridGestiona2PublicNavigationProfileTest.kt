...
class MadridGestiona2PublicNavigationProfileTest {
    private val profileId = ProfileId("comunidad-madrid-gestiona2-public-navigation")
    private val portalId = PortalId("comunidad-madrid-gestiona2")
    private val startUrl = URI(
        "https://gestiona2.comunidad.madrid/gpse_solicitud/accesos.jsf?numref=2094",
    )

    @Test
    fun profileConfigPreservesExactQaOnlyNavigationAndMobileLimitation() {
        val profile = BuiltInSiteProfiles.catalog.profiles.single { it.profileId == profileId }

        assertEquals(1, profile.profileVersion)
        assertEquals(
            "Comunidad de Madrid — gestiona2 / navegación pública",
            profile.displayName,
        )
        ...
        assertEquals(
            setOf(ExactOrigin.parse("https://gestiona2.comunidad.madrid")),
        ...
        assertEquals(
            listOf(
                ...
                EvidenceReference(
                    URI("https://gestiona2.comunidad.madrid/gpse_solicitud/index_seleccion.jsf"),
    ...
    fun registryResolvesExactEntryAndSameOriginBrowsePathsInQa() {
        ...
        listOf(
            "http://gestiona2.comunidad.madrid/gpse_solicitud/accesos.jsf?numref=2094",
            "https://gestiona2.comunidad.madrid:8443/gpse_solicitud/accesos.jsf?numref=2094",
            "https://evil.gestiona2.comunidad.madrid/gpse_solicitud/accesos.jsf?numref=2094",
            "https://comunidad.madrid/gpse_solicitud/accesos.jsf?numref=2094",
            "https://user@gestiona2.comunidad.madrid/gpse_solicitud/accesos.jsf?numref=2094",
        ).forEach { rejectedUrl ->
    ...
    @Test
    fun securityAndProtocolIsolationExposesNoMobileSigningOrCertificateBridge() {
    ...
    @Test
    fun catalogBindsMadridGestiona2AsPendingNavigationAndResolvesExactLaunchOnly() {
        val publicCatalog = loadBundledPublicPortalCatalog()
        val entry = publicCatalog.entries.single { it.inventoryId == "ES-PUB-0013" }

        assertEquals(portalId, entry.portalId)
        assertEquals(profileId, entry.profileId)
        assertEquals(startUrl, entry.entryUrl)
        assertEquals("MADRID_GESTIONA2_PUBLIC_NAVIGATION", entry.protocolFamily)
        ...
        assertTrue(entry.limitations.contains("móvil", ignoreCase = true))
        ...
        listOf(
            ...
            "https://gestiona2.comunidad.madrid:8443/gpse_solicitud/accesos.jsf?numref=2094",
            "https://evil.gestiona2.comunidad.madrid/gpse_solicitud/accesos.jsf?numref=2094",
            "https://gestiona2.comunidad.madrid/gpse_solicitud/index_seleccion.jsf",
        ).forEach { rejectedUrl ->
