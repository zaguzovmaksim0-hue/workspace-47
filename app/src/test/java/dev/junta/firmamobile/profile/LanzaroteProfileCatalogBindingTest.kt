...
class LanzaroteProfileCatalogBindingTest {
    private val profileId = ProfileId("lanzarote-sede-public-navigation")
    private val portalId = PortalId("lanzarote-sede-electronica")
    private val startUrl = URI("https://cabildodelanzarote.sedelectronica.es/info.0")
    private val redirectOrigin = ExactOrigin.parse("https://lanzaroteylagraciosa.sedelectronica.es")
    private val redirectUrl = URI("https://lanzaroteylagraciosa.sedelectronica.es/info.0")

    @Test
    fun profileConfigPreservesExactEntryAndExplicitRedirectContract() {
        val profile = BuiltInSiteProfiles.catalog.profiles.single { it.profileId == profileId }

        assertEquals(1, profile.profileVersion)
        assertEquals(
            "Sede electrónica pública del Cabildo Insular de Lanzarote",
            profile.displayName,
        )
        ...
        assertEquals(
            setOf(ExactOrigin.parse("https://cabildodelanzarote.sedelectronica.es")),
            profile.initiatorOrigins,
        )
        assertEquals(setOf(redirectOrigin), profile.redirectOrigins)
        ...
        assertEquals(
            listOf(
                EvidenceReference(startUrl, LocalDate.parse("2026-08-25")),
                EvidenceReference(redirectUrl, LocalDate.parse("2026-08-25")),
            ),
            profile.evidence,
        )
    }
    ...
    @Test
    fun registryResolvesExactEntryAndOnlyExplicitRedirectOriginInQa() {
        ...
        assertNull(BuiltInSiteProfiles.qaRegistry.resolve(redirectUrl))

        listOf(
            "http://cabildodelanzarote.sedelectronica.es/info.0",
            "https://cabildodelanzarote.sedelectronica.es:8443/info.0",
            "https://evil.cabildodelanzarote.sedelectronica.es/info.0",
            "https://user@cabildodelanzarote.sedelectronica.es/info.0",
            "https://lanzaroteylagraciosa.sedelectronica.es:8443/info.0",
            "https://evil.lanzaroteylagraciosa.sedelectronica.es/info.0",
        ).forEach { rejectedUrl ->
    ...
    fun securityAndProtocolIsolationEnforcesFailClosedNavigation() {
        ...
        assertEquals(
            setOf(
                ExactOrigin.parse("https://cabildodelanzarote.sedelectronica.es"),
                redirectOrigin,
            ),
    ...
    @Test
    fun catalogBindsLanzaroteAsPendingNavigationAndResolvesOnlyExactLaunch() {
        val publicCatalog = loadBundledPublicPortalCatalog()
        val entry = publicCatalog.entries.single { it.inventoryId == "ES-PUB-0136" }
        ...
        assertNull(qa.resolveLaunch(profileId, redirectUrl))
        ...
        listOf(
            ...
            "https://lanzaroteylagraciosa.sedelectronica.es/info.0",
        ).forEach { rejectedUrl ->
