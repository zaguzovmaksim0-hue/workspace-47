...
class ZamoraProfileCatalogBindingTest {
    private val profileId = ProfileId("zamora-sede-public-navigation")
    private val portalId = PortalId("diputacion-zamora-sede")
    private val startUrl = URI("https://diputaciondezamora.sedelectronica.es/info.0")
    ...
    fun profileConfigPreservesExactFailClosedNavigationContract() {
        ...
        assertEquals(
            "Sede electrónica pública de la Diputación Provincial de Zamora",
            profile.displayName,
        )
        ...
        assertEquals(
            setOf(ExactOrigin.parse("https://diputaciondezamora.sedelectronica.es")),
    ...
    fun registryResolvesExactEntryAndSameOriginBrowsePathsInQa() {
        ...
        assertEquals(
            ...
            BuiltInSiteProfiles.qaRegistry.resolve(
                URI("https://diputaciondezamora.sedelectronica.es/info"),
            )?.trustMode,
        )

        listOf(
            "http://diputaciondezamora.sedelectronica.es/info.0",
            "https://diputaciondezamora.sedelectronica.es:8443/info.0",
            "https://evil.diputaciondezamora.sedelectronica.es/info.0",
            "https://sedelectronica.es/info.0",
            "https://user@diputaciondezamora.sedelectronica.es/info.0",
        ).forEach { rejectedUrl ->
    ...
    fun securityAndProtocolIsolationEnforcesFailClosedNavigation() {
        ...
        assertEquals(
            setOf("diputaciondezamora.sedelectronica.es"),
            JuntaOriginPolicy.browserAllowedHosts(profileId),
        )
    }

    @Test
    fun catalogBindsZamoraAsPendingNavigationAndResolvesLaunchOnlyForExactEntry() {
        val publicCatalog = loadBundledPublicPortalCatalog()
        val entry = publicCatalog.entries.single { it.inventoryId == "ES-PUB-0177" }

        assertEquals(portalId, entry.portalId)
        assertEquals(profileId, entry.profileId)
        assertEquals(startUrl, entry.entryUrl)
        assertEquals("PROVINCIAL_SEDE_PUBLIC_NAVIGATION", entry.protocolFamily)
        ...
        assertEquals(
            ...
            entry.observedMechanisms.map { it.name }.toSet(),
        )
        ...
        listOf(
            ...
            "https://diputaciondezamora.sedelectronica.es:8443/info.0",
            "https://evil.diputaciondezamora.sedelectronica.es/info.0",
            "https://diputaciondezamora.sedelectronica.es/",
        ).forEach { rejectedUrl ->
