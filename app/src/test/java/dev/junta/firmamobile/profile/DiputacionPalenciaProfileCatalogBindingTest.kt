...
class DiputacionPalenciaProfileCatalogBindingTest {
    private val profileId = ProfileId("diputacion-palencia-sede")
    private val portalId = PortalId("diputacion-palencia-sede")
    private val startUrl = URI("https://sede.diputaciondepalencia.es")
    ...
    fun `profile exposes only the exact reviewed browse surface`() {
        ...
        assertEquals(setOf(ExactOrigin.parse("https://sede.diputaciondepalencia.es")), profile.initiatorOrigins)
        ...
        listOf(
            ...
            "https://user@sede.diputaciondepalencia.es",
            "https://sede.diputaciondepalencia.es.evil.example",
            "https://sede.diputaciondepalencia.es:8443",
        ).forEach { rejected ->
    ...
    fun `catalog binding does not invent signature capabilities`() {
        ...
        assertEquals("ES-PUB-0166", entry.inventoryId)
