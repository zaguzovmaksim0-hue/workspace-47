(() => {
  ...
  function interceptMiniAppletSign(args) {
    ...
    if (args.length !== 6 || typeof successCallback !== "function" ||
        ...
        (!isJuntaCades && !isRegXades && !isExactUgrLiteralCall &&
          ...
          !isExactCdtiCall && !isExactPoliciaCall && !isExactGranCanariaCall && !isExactXuntaCall)) {
      rejectDirectCall(errorCallback, "INVALID_REQUEST");
      return true;
    }
    if (!bridge || typeof bridge.postMessage !== "function") {
  ...
  if (!miniAppletDescriptor || miniAppletDescriptor.configurable === true) {
    let miniApplet = wrapMiniApplet(window.MiniApplet, false, false, xSel, false);
    Object.defineProperty(window, "MiniApplet", {
      ...
      set(value) {
        miniApplet = wrapMiniApplet(value, false, false, xSel, false);
      }
    });
    window.addEventListener("DOMContentLoaded", () => {
      miniApplet = wrapMiniApplet(miniApplet, false, false, xSel, false);
    }, { once: true });
  } else {
    wrapMiniApplet(window.MiniApplet, false, false, xSel, false);
  }
