...
object AfirmaJavascriptShim {
    ...
    fun load(
        ...
        juntaMultiModeCompatibilityEnabled: Boolean = false,
    ): String {
        ...
        check(script.countOccurrences(JUNTA_MULTIMODE_COMPATIBILITY_PLACEHOLDER) == 1)
        val configured = script
            ...
            .replace(
                JUNTA_MULTIMODE_COMPATIBILITY_PLACEHOLDER,
                if (juntaMultiModeCompatibilityEnabled) "true" else "false",
            )
    ...
    private const val JUNTA_MULTIMODE_COMPATIBILITY_PLACEHOLDER =
        "__JFM_JUNTA_MULTIMODE_COMPATIBILITY_ENABLED__"
}
