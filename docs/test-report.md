...
## Autonomous G16-01 — certificate unlock same-boot monotonic lease — 2026-08-08

The G16 trust-time lead is resolved by a same-device-boot recovery contract. A reboot invalidates automatic recovery and requires password re-entry. `JFMUC002` AAD binds boot count, original civil issue/expiry, certificate reference and the **original manual-session elapsed-realtime observation**; `CertificateSession` and the Android cache both use `SystemClock.elapsedRealtimeNanos()` in production. Slow cache IO therefore cannot reset the persisted authorization horizon. Civil rollback cannot extend either in-memory identity or persisted recovery, while civil expiry remains a conservative secondary cap. Cache restore hands only the authenticated remaining lease to the session before gateway reload.

Initial TDD established the missing session/cache/restore contracts. Independent review then found the delayed-persistence-origin issue before commit and drove an additional RED `job_20260808_214657_077bd47b`; focused GREEN `job_20260808_215055_9273b77a` passed after the minimum API/AAD fix. Extra branch tests cover elapsed rollback, unavailable boot-time evidence, legacy magic, exact session boundary and monotonic rollback. The separate generation-clear review concern was classified non-defect at the restore linearization point plus production cancellation-before-clear behavior; a second reviewer timed out without verdict and is not counted as evidence.
