...
| Launcher PNGs and other custom visual identity resources | Present as project application resources; focused audit identifies the custom binary visual set separately | `PROVENANCE_UNRESOLVED` | **Publication blocker until audited/resolved.** Replace/remove or establish source/redistribution rights before blanket project licensing. |
...
3. Complete the focused source review/maintainer attestation for `afirma_shim.js` and other project-owned source so that project licensing authority is explicit.
4. Complete the final full-history secret scan and final synchronization checks on the publication candidate.
