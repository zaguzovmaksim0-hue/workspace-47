# ITJ public sede navigation evidence — 2026-08-25

## Scope

This review covered only a public unauthenticated `GET` of the official ITJ
electronic-sede root:

- URL: `https://sede.transicionjusta.gob.es/`
...
Workspace-47 adds the QA-only profile `itj-sede-public-navigation` with the
exact root URL and exact origin `https://sede.transicionjusta.gob.es`. The
profile exposes ordinary same-origin browsing only. It declares no
...
Primary source: <https://sede.transicionjusta.gob.es/>.
