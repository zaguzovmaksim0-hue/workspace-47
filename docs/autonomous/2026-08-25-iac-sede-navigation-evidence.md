# IAC public sede navigation evidence — 2026-08-25

## Scope

This review covered only a public unauthenticated `GET` of the official IAC
electronic-sede root:

- URL: `https://iac.sede.gob.es/`
- transport: HTTPS, normal certificate validation, no TLS bypass
- observed result: HTTP 200, stable public HTML response
...
Workspace-47 adds the QA-only profile `iac-sede-public-navigation` with the
exact root URL and exact origin `https://iac.sede.gob.es`. The profile exposes
...
Primary source: <https://iac.sede.gob.es/>.
