...
4. The facade can continue the same TLS request through the bounded `ws235-1` … `ws235-6` cluster paths. The continuation retains the exact `action`, `appId`, callback, `ticketId`, and `webSessionId` tuple from the confirmed request.
5. The public no-certificate failure path returns through `/auth/returnLogin`, `/auth/endLogin`, then VEA `/authFacade` with an error result. The current public VEA bundle also handles the successful `/authFacade` shape with `token` plus `redirectUrl`.

Runtime trust is deliberately narrower than each origin. `api-veaja` is accepted only for the exact source/return paths and query contracts above. `ws235` and a cluster continuation are accepted only for the exact VEA source-to-target transition and the same confirmed TLS tuple. Any changed host/path pairing, application, callback, ticket, session, extra parameter, port, or scheme fails closed. The client certificate challenge is handled in the same WebView so the public authentication session is not copied between WebViews.
