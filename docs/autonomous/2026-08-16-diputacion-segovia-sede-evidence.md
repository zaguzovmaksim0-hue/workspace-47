# Sede electrónica de Diputación de Segovia — Public BROWSE_ONLY Evidence — 2026-08-16
...
- Inventory: `ES-PUB-0169` / `diputacion-segovia-sede`.
- Exact catalog seed: `https://sede.dipsegovia.es`.
...
The exact catalog seed is enabled only as `BROWSE_ONLY`. The runtime profile trusts only `https://sede.dipsegovia.es` as its initiator origin and grants no privileged capabilities, operation policies, endpoints, redirect origins, trusted browse origins, or client-auth policy. Inventory status remains `BROWSE_ONLY`.
